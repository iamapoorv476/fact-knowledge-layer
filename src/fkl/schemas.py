...
2. **Deterministic identity.** 'AtomicFact.id' and 'FactRelationship.id' are
   ...
   is what makes incremental ingestion(add a 4th PDF without rebuilding the graph)
   safe and idempotent.
...
3. **Reconciliation lives in the schema, not in prompts.** Entity/attribute
   ...
   Python comparisions instead of asking a model twice.
...
class PeriodType(str, Enum):
    ...
    _DMY_RE = re.compile(r"\b(\d{1,2})(?:st|nd|rd|th)?\s+([a-z]{3,9})\.?,?\s+(\d{4})\b", re.I)
...
class TemporalScope(BaseModel):
    """Machine-comparable projection of an extracted temporal expression."""
...
class SourceAnchor(BaseModel):
    """Deterministic character indices into the canonical text of a parsed page."""
